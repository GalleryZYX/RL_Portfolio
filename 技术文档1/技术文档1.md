...
| 文件路径 | 功能说明 |
|---------|---------|
| `envs/portfolio_env.py` | 交易环境核心代码，包含状态转移、收益计算、交易成本扣除逻辑 |
...
该命令将安装 Gymnasium、Pandas、NumPy、Matplotlib 等当前阶段必需依赖，版本与开发机本地环境严格一致。
...
1. **数据文件不上传仓库。** `data/` 目录已被 `.gitignore` 排除。后续下载的股票数据及 API Token 相关文件统一存放在清华云盘中（Readme.md中有链接），避免仓库体积膨胀及敏感信息泄露。
